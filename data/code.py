...
class CodeConfig:
    ...
    def load_config(self, config_path):
        ...
        # Windows: use repo-relative paths so /data and /opt work
        if os.name == "nt":
            base = Path(config_path).resolve().parent.parent
            config["code_store"]["source_code_dir"] = str(base / "sandbox_codes")
            java_base = str(base / "opt" / "java_libs")
            for lang_config in config.get("supported_languages", {}).values():
                for key in ("compile_flags", "execute_flags"):
                    if key not in lang_config or not isinstance(lang_config[key], list):
                        continue
                    out = []
                    for s in lang_config[key]:
                        s = s.replace("/opt/java_libs", java_base)
                        # Java classpath on Windows uses ";" not ":"; preserve drive letter (e.g. E:)
                        if ":" in s:
                            s = s.replace(":", ";")
    ...
    def get_language_config(self, language):
        ...
        language_config = dict(self.supported_languages[language])
        language_config["lang"] = language
        language_config["source_code_dir"] = os.path.join(self.source_code_dir, language)
...
class CodeStore:
    ...
    def build_code_env(self, request_data):
        ...
        code_dir = os.path.join(language_config["source_code_dir"], uuid.uuid4().hex)
        if not os.path.exists(code_dir):
            os.makedirs(code_dir)
        code_path = os.path.join(code_dir, language_config["file_name_template"])
