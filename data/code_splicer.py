...
class CodeSplicer:
    ...
    def splice_java_code(self, code: str, assertions: str) -> str:
        ...
        Returns:
            处理后的合并代码
        """
        # 将主函数中的public class，private class 和 protected class都转成class（单文件只能有一个public类，文件名为test.java）
        main_code = code.replace("public class", "class").replace("private class", "class").replace("protected class", "class")
        # 测试类也要改为package-private，否则 javac 报错 "public class XTest should be in XTest.java"
        assertions = assertions.replace("public class", "class").replace("private class", "class").replace("protected class", "class")
